...
4. Sync / deploy once and verify functions `generate-script`, `synthesize-segment`, `reconcile-subscription-resets` (hourly cron; Stripe-verified fallback when `invoice.paid` is missed), and `stuck-script-reaper` (every 5 min) appear in the Inngest UI.
