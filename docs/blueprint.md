...
| Auth, DB, Realtime | Supabase         | Email/password + password reset (`/forgot-password` → `resetPasswordForEmail` with `redirectTo=…/reset-password` → PKCE `?code=` exchanged via `exchangeCodeForSession`, then `updateUser({ password })`). Mis-targeted `/?code=` is forwarded to `/reset-password`. Canonical origin `NEXT_PUBLIC_APP_URL` (`https://phaseshift.app` in prod). |
