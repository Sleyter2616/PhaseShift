...
| Auth, DB, Realtime | Supabase         | Email/password + password reset (`/forgot-password` → `resetPasswordForEmail` → `/reset-password`); Realtime channel per script for synthesis progress. Canonical origin `NEXT_PUBLIC_APP_URL` (`https://phaseshift.app` in prod). |
